...
def init_args():
    ...
    parser.add_argument("--task", default='eae', type=str,)
    parser.add_argument("--dataset",default='ere',type=str,help="The name of the dataset, selected from: [ACE05, ere]")
    parser.add_argument(
        "--eval_data_split",
        default='test',
        choices=["test", "dev"],
    ...
    parser.add_argument("--single_perspective_type",
                    default="vote",
                    choices=["rank", "rand", "heuristic", "vote"],
                    type=str)
    ...
    parser.add_argument("--data_ratio_flag",
                        default=1.0,
                        type=float,
...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        dataloader = DataLoader(
            ...
            drop_last=True
            if args.data_ratio_flag > 0.3 else False, 
            shuffle=True,
            num_workers=2)
...
def evaluate(model, task, data, data_type):
    ...
    if args.load_path_cache:
        ...
        error_count = 0
        error_sample_id_collect_list = []

        for batch in tqdm(data_loader):


            sample_id = int(len(targets) // 4)
            print("\nsample_id: ", sample_id)
            print("One sample input: ", model.tokenizer.decode(batch["source_ids"][0], skip_special_tokens=True))
            ...
            try:
                ...
                error_count += 1
                error_sample_id_collect_list.append(sample_id)
                print(f"\nError occurred in sample number: {sample_id}, Error: {e}")


                length = len(batch['source_ids'])
                dec_outputs.extend(['' for _ in range(length)])
                outputs.extend(['' for _ in range(length)])
                targets.extend(['' for _ in range(length)])
                probs.extend([0 for _ in range(length)])


        print("\nGenerate Error Sample Numbers : ", error_count)
        print("Generate Error Sample IDs : ", error_sample_id_collect_list)
...
def train_function(args):
    ...
    if args.do_train:
        print("\n", "=" * 30, f"NEW EXP: {args.task} on {args.dataset}",
              "=" * 30, "\n")
        tokenizer = T5Tokenizer.from_pretrained(args.model_name_or_path, local_files_only=True if args.model_name_or_path not in ["google-t5/t5-large","google-t5/t5-base"] else False)
        ...
        tfm_model = Gems.from_pretrained(
    ...
    if args.do_inference:
        ...
        tfm_model = Gems.from_pretrained(model_path, head = args.head)
        model = T5FineTuner(args, tfm_model, tokenizer)
